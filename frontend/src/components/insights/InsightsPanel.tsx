  ...
  const generateInsights = async () => {
    ...
    try {
      ...
      if (response.podcasts && response.podcasts.length > 0) {
        console.log('Podcasts generated successfully:', response.podcasts);

        
        // Store podcasts data in state for playback
        dispatch({ type: 'SET_PODCASTS', payload: response.podcasts });
      } else {
        console.log('No podcasts data in response:', response);
      ...
      if (state.pdfs.length > 0) {
        ...
        const processSelectedText = async () => {
          try {
            ...
            // Fallback: generate job insights from the selected text without requiring PDFs
            try {
              console.log('Falling back to job insights generation from selected text...');
              const jobResponse = await apiService.generateJobInsights(state.selectedTextFromPDF || '');
              if (jobResponse) {
                setJobInsightData(jobResponse);
                // Flip job card to show insights
                if (!isJobCardFlipped) {
                  setIsJobCardFlipped(true);
                }
                showToast({
                  message: 'Generated insights from selected text (fallback).',
                  type: 'success'
                });
              } else {
                showToast({
              ...
              console.error('Fallback job insights generation failed:', fallbackError);
              showToast({
                message: 'Failed to generate insights from selected text. Please try again.',
                type: 'error'
              });
      ...
      <div className="flex items-start justify-between mb-2">
        ...
        <div className="flex items-center space-x-2 flex-shrink-0 ml-2">
          ...
          <button
            onClick={async (e) => {
              e.stopPropagation();
              console.log('Podcast button clicked for card (front):', insight.id);
              
              // Find the podcast for this card
              const cardPodcast = state.podcasts.find(p => p.cardId === insight.id);
              console.log('Found podcast (front):', cardPodcast);
              
              if (cardPodcast) {
                // Podcast exists, flip to podcast view
                setPodcastViewCardId(insight.id);
                  } else {
                console.log('No podcast found for card (front):', insight.id);
                console.log('Generating podcast on-demand...');
              ...
              onClick={async () => {
                if (jobToBeDone.trim() && state.pdfs.length > 0) {
                  ...
                  if (jobPodcast) {
                    ...
                    // Generate podcast on-demand
                    console.log('No job podcast found, generating on-demand...');
